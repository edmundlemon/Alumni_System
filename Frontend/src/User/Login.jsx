...
export default function LoginPage({ initialForm = "login" }) {
  ...
  const [formResetError, setFormResetError] = useState({
    email: "",
    password: "",
    ...
    axios
      ...
      .catch((error) => {
        ...
        if (error.response && error.response.status === 422) {
          console.error("Validation errors:", error.response.data.errors);
            const meesege = (error.response.data.errors);
            resetPassPost.email = meesege
        } else {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {currentForm === "resetPassword" ? (
        <div className="absolute top-7 left-20 text-white text-5xl font-extrabold">
          Reset Password
        </div>
      ) : (
        <div className="absolute top-7 left-20 text-white text-5xl font-extrabold">
            ...
            <div className="w-[320px] p-10 flex flex-col justify-center bg-white">
              ...
              <form className="space-y-6" onSubmit={handleLoginSubmit}>
                ...
                <button
                  ...
                  className={`w-full py-2 text-white font-bold rounded-md transition duration-300 shadow-md flex justify-center items-center ${
                    isSubmitting
            ...
            <div className="w-[320px] p-10 flex flex-col justify-center bg-white">
              ...
              <form className="space-y-6" onSubmit={handleResetSubmit}>
                <div className="flex flex-col">
                  ...
                  <input
                    ...
                    value={resetPassPost.email}
                     style={{
                        border: formResetError.email ? "1px solid red" : "",
                      }}
                    onChange={handleInput}
                    ...
                    required
                  />
                  <p className="text-red-600 text-xs">{formResetError.email}</p>
                  <div
                    className="text-xs text-blue-600 hover:underline ml-auto mt-4 cursor-pointer"
                    onClick={() => {setCurrentForm("login"),setIsSubmitting(false)}}
                  >
                    Back to Login
                  </div>
                </div>
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className={`w-full py-2 font-bold text-white rounded-md transition duration-300 shadow-md flex justify-center items-center ${
                    isSubmitting
                      ? "bg-blue-700 cursor-not-allowed"
                      : "bg-blue-900 hover:bg-blue-800 "
                  }`}
